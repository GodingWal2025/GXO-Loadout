...
- **Shift handoff**: The home screen lists unfinished loads, recorded progress, next actions, and the latest handoff note. Notes and inspector transfers are retained across devices, including concurrent note additions.
...
- **Evidence package**: Download a ZIP containing a printable HTML report, referenced document/pallet images, the inspection JSON, and a manifest. Missing images are reported in both the export and the UI; reconnect and export again to retrieve unavailable cloud images. Resolution notes preserve context without automatically clearing the underlying flags.
