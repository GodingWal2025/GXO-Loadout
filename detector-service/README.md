...
`DETECTOR_SERVICE_URL` is the only pallet-vision backend the Function proxies to.
Leave it unset and both `/api/analyze-pallet-count` and `/api/analyze-pallet-faces`
return 501, and the app degrades cleanly to manual layer entry.
